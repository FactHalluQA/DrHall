...
def fact(question: Question) -> list[ChatSlice]:
    """
    ...
    chat_slice = fact_step_1(question)
...
def fact_step_1(question: Question, prompt: str = "") -> ChatSlice:
    if not prompt:
        prompt = f"【{question.question} {question.english}】, is this statement not true?"
    chat_slice = util.create_chat_slice(prompt=prompt)
    question.fact = chat_slice.answer
    return chat_slice
