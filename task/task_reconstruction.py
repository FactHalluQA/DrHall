...
def reconstruction(question: Question) -> list[ChatSlice]:
    """
    Question：【源问题】
    Answer：【源回答】
    ...
    question: 问题
    """
    if question.english_phrase and question.similar_phrases and question.reconstruction:
        return []

    chat_slice_1 = reconstruction_step_1(question)
    ...
    chat_slice_2 = reconstruction_step_2(question)
    ...
    chat_slice_3 = reconstruction_step_3(question)

    logger.bind(question=question, detail='prompt').debug(f'"{chat_slice_3.prompt}"')
    logger.bind(question=question, detail='answer').debug(f'"{question.reconstruction}"')

    return [chat_slice_1, chat_slice_2, chat_slice_3]


def reconstruction_step_1(question: Question, prompt: str = "") -> ChatSlice:
    if not prompt:
        prompt = \
            (f"Question：【{question.question}】 Answer：【{question.english}】 "
...
def reconstruction_step_2(question: Question, prompt: str = "") -> ChatSlice:
    if not prompt:
        prompt = \
...
def reconstruction_step_3(question: Question, prompt: str = "") -> ChatSlice:
    if not prompt:
        prompt = \
    ...
    question.reconstruction = chat_slice_3.answer
    return chat_slice_3
