...
def repeat(question: Question) -> list[ChatSlice]:
    """
    ...
    question: 问题
    """
    if question.polished_question and question.repeat:
        return []

    chat_slice_1 = repeat_step_1(question)
    ...
    chat_slice_2 = repeat_step_2(question)
...
def repeat_step_1(question: Question, prompt: str = "") -> ChatSlice:
    if not prompt:
        prompt = \
...
def repeat_step_2(question: Question, prompt: str = "") -> ChatSlice:
    if not prompt:
        prompt = f"{question.polished_question}"
    chat_slice_2 = util.create_chat_slice(prompt=prompt)
    question.repeat = chat_slice_2.answer
    return chat_slice_2
